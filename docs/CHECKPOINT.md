...
- **`SqlArrayMapping { From, ElementColumn }`** — come una collezione M2M diventa un `EXISTS` correlato (tabella ponte + join + correlazione col padre). Vive **nell'adapter SQL**, non nei metadati store-agnostic (è forma relazionale = infrastruttura, come il fluent mapping di EF nel DbContext). **Unica fonte di verità**: lo stesso `SqlArrayMapping` alimenta sia il filtro `EXISTS` sia la proiezione dell'array via `json_group_array`.
- **`SqlSearchQueryBuilder`** — assembla la query completa parametrizzata: `SELECT` (proiezione; array → subquery `json_group_array`), `FROM` base, `WHERE` (base soft-delete AND filtro), `ORDER BY`, `LIMIT @take OFFSET @skip`. Ritorna `SqlQueryPlan { Sql, Parameters, Fields }` (+`BuildCount`). Paginazione con parametri **nominali** (`@skip/@take`) per non collidere coi `@pN` posizionali del filtro.
- **`SqlSearchExecutor`** — esegue una `SqlQuery` su **qualsiasi `DbConnection` ADO.NET** (SQLite ora, Npgsql poi): `Query` (righe→dizionari per alias) + `Count`. Valori sempre legati come parametri.
- **`SqlEntitySchema` + `ISqlSchemaProvider`** (impl: **`CatalogSqlSchemaProvider`**) — la config SQL per-entità (`From`, `BasePredicate`, `ArrayMappings`) **centralizzata** e recuperata **per nome** (`sqlSchemas.GetSchema("brand")`), separata dai metadati store-agnostic. **Hardcodata in codice** (non da DB): lo schema — tabella/alias/tabella-ponte/soft-delete — è struttura, cambia solo con una migrazione/deploy → sta nel codice come il fluent mapping di EF nel `CatalogDbContext`. (Le *definizioni dei campi* restano invece data-driven.) Flusso: entità → `GetEffectiveMap` (campi, per permessi/tenant) **+** `GetSchema` (binding SQL) → `new SqlSearchQueryBuilder(map, schema)`.
...
- **Array JSON (scalari/oggetti)** → `SqlArrayMapping` con `From` = unnest `FROM jsonb_array_elements[_text](coalesce("Data"->'tags','[]'::jsonb)) AS elem WHERE true`; `ElementColumn` = `elem` (scalari) o `e ->> 'sku'` (oggetti). **Stessa macchina EXISTS della M2M → zero codice nel translator.**
- **Proiezione** → unico vero ritocco: `SqlArrayMapping` ha ora `Projection` (opzionale). `null` ⇒ M2M ricostruita con `json_group_array`; valorizzato ⇒ array JSON proiettato **diretto** (`"Data" -> 'tags'`). `SqlSearchQueryBuilder.SelectColumn` usa `Projection ?? json_group_array(...)`.
...
## Revisione architetturale (2026-07-14) ✅
Revisione multi-agente su 3 domande (assembly / naming / leggibilità): 17 rilievi confermati, **tutti media/bassa, zero bloccanti**. Nessun leak tecnico nel motore. **Batch manutenibilità applicato:**
- **Default sort uniforme**: policy unica `SearchMapExtensions.DefaultSortField()` (id → createdAt → null); ora **anche `SqlSearchQueryBuilder` e `MongoSearchExecutor`** applicano il default quando manca il sort → paginazione deterministica coerente coi tre store (prima solo LINQ/EF).
- **Dedup**: `SearchMapExtensions.DefaultProjection()` (regola "proiezione vuota → visibili" prima in 4 punti); `LinqSearchExecutor.ApplyOrder()` (reflection OrderBy prima triplicata); `FieldDescriptor.SqlColumn()` extension (prima duplicata tra translator e builder).
- **Rinomine**: `SqlArrayMapping` (ex SqlArrayFilter, porta anche la proiezione) + `SqlEntitySchema.ArrayMappings`; `SqlSearchExecutor` (ex RawSqlSearchExecutor, simmetrico a Linq/Mongo); `SqlQueryPlan` (ex SqlQuery, gemello di MongoQueryPlan); `SearchFieldDefinitionResolver.ResolveSelectorBased`/`ResolveStoragePathBased` (ex Relational/Document — il criterio reale è selettore-vs-storagepath, non relazionale-vs-documentale).
- **Cosmetici**: `SearchEntity.RelationalRaw` non più generico (il ClrType non serve al Raw); doc `StoreKind.PostgresRaw` completato; sezione demo SPLIT QUERY ripristinata (`.AsSplitQuery()` era commentato); `//LenghtMm` e `<param>` vuoto sistemati.
- **Rimandato (solo nota di doc)**: `IProductRepository` fuori dal motore (documentato nel file, da estrarre solo se il motore diventa libreria a sé — mai in Api); criterio di collocazione degli adapter (LINQ in Application per riusabilità in-memory, documentato in `LinqSearchExecutor`); spostamento del seed `SimulatedFieldDefinitionDatabase` quando le definizioni andranno a DB.

Build verde (0 errori, 0 warning CS). Prossimo: **fase di testing**.
